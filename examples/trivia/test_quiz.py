...
import examples.custom_components.personality_quiz_metric as pqm
...
def run_quiz(scenario, year, agent_source = False, start = 0):
    ...
    if not agent_source:
        ...
        for i in range(N_AGENTS):
            print(f'Creating agent {i+1} of {N_AGENTS}...')
        ...
        clock = build_agents.build_clock(year)
        N_AGENTS = len(os.listdir(f'./agents/{year}/{scenario}'))
    ...
    if start == 0:
        if scenario == 'personality':
            firstrow = ['agent_id', 'agent_name', 'o_score', 'c_score', 'e_score', 'a_score', 'stability_score', 'o_gt', 'c_gt', 'e_gt', 'a_gt', 'n_gt', 'run']
        else:
            firstrow = ['agent_id', 'agent_name', 'traits', 'question', 'answer', 'correct', 'run']

        with open(f'./data/llama2_{scenario}_{year}_agents.csv', 'w') as newfile:
    ...
    for i in range(start, N_AGENTS):
        ...
        for j in range(N_RUNS):
            ...
            print(f'Testing run {j+1}: Agent {i+1} out of {N_AGENTS}')
            ...
            if scenario == 'personality':
                ...
                quiz = qm
            quiz_metric = quiz.QuizMetric(model, agent, file_path, measurements = measurements, verbose = True)
            results = quiz_metric.observe(context, return_data=True)

            with open(f'./data/llama2_{scenario}_{year}_agents.csv', 'a') as newfile:
                writer = csv.writer(newfile, delimiter=',', quoting=csv.QUOTE_MINIMAL)
                if scenario == 'personality':
                    results.append(j)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--scenario', default = 'personality', type = str, help = 'Name of the scenario to collect data for.')
    parser.add_argument('-y', '--year', default = 2024, type = int, help = "Year the agents are from.")
    parser.add_argument('-n', '--number', default= 0, type = int, help = "Number agent to start from.")
    parser.add_argument('--prebuilt', action = 'store_true')
    args = parser.parse_args()
    
    run_quiz(args.scenario, args.year, agent_source = args.prebuilt, start = args.number)